...
export default {
  ...
  mounted () {
    ...
    this.initChart()
    // this.getData()
    // 原来获取数据的地方，改为发送数据到服务器，告诉服务器现在需要的数据
    this.$socket.send({
      action: 'getData',
      socketType: 'sellerData',
      chartName: 'seller', // 此处参看'../data'下的json名称
      value: ''
    })
  ...
  methods: {
    ...
    initChart () {
      ...
      this.chartInstance = this.$echarts.init(this.$refs.seller_ref, this.theme) // this.theme是vuex中的
